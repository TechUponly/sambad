...
class _LoginPageState extends State<LoginPage> {
    ...
    // Country data: code, name, digit count
    final List<Map<String, dynamic>> _countries = [
      {'name': 'India', 'code': '+91', 'digits': 10},
      {'name': 'United States', 'code': '+1', 'digits': 11},
      {'name': 'United Kingdom', 'code': '+44', 'digits': 10},
      {'name': 'Bangladesh', 'code': '+880', 'digits': 10},
      {'name': 'Nepal', 'code': '+977', 'digits': 10},
      {'name': 'Pakistan', 'code': '+92', 'digits': 10},
      {'name': 'Sri Lanka', 'code': '+94', 'digits': 9},
      // ...add more as needed
    ];
    int _selectedCountry = 0; // Default to India

    // Simulate admin config for OTP (should be fetched from backend/config in real app)
    final bool _otpEnabled = false;
    ...
    void _sendOtp() {
      ...
      if (phone.length != country['digits']) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Enter a valid mobile number (${country['digits']} digits) for ${country['code']}')),
        );
        return;
      }
      setState(() {
                        ...
                        if (!_otpEnabled) ...[
                            ...
                            children: [
                              SizedBox(
                                ...
                                child: DropdownButtonFormField<int>(
                                  ...
                                  items: List.generate(_countries.length, (i) {
                          ...
                          ElevatedButton(
                            onPressed: () {
                              final country = _countries[_selectedCountry];
                              final phone = _phoneController.text.replaceAll(RegExp(r'\D'), '');
                              if (phone.isEmpty || phone.length != country['digits']) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(content: Text('Please enter a valid mobile number (${country['digits']} digits) for ${country['code']}')),
                                );
                                return;
                              }
                              // Optionally, add more country-specific validation here
                              Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (_) => const HomePage()));
                            },
                            ...
                            child: const Text('Continue', style: TextStyle(fontSize: 18, color: Colors.white, fontWeight: FontWeight.bold, letterSpacing: 0.7, shadows: [Shadow(color: Colors.black38, blurRadius: 2)])),
                          ),
